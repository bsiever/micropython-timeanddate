import utime             # For ticks

# External API

# Set date at given cpuTime
def setDate(mm, dd, yy):
    cpuTime = utime.ticks_ms()//1000
...
def time():
    return timeFor(utime.ticks_ms()//1000)

def timestamp():
    (mm, dd, yy, hh, min, sec, _) = time()
    return "{:04d}-{:02d}-{:02d} {:02d}:{:02d}:{:02d}".format(yy,mm,dd, hh, min, sec)
